...
def generate_feature_graph(which_model, pred, mod):
    print(FEATURES_DICT[f][0])
    print(FEATURES_DICT[f][1])
    print(FEATURES_DICT[f][2])

    
    model_path = '../../models/exp2_models/' + pred + '/' + mod + '_' + which_model + 'model.joblib.pkl'
    ...
    feature_importances = [(FEATURES_DICT[which_model][0][i], list(importances)[i], FEATURES_DICT[which_model][1][i]) for i in range(len(FEATURES_DICT[which_model][0]))]
    ...
    plt.savefig('../../report_output/exp2/feature_maps/' + pred +'/' + mod + '_' + which_model + 'feature.png')
