...
test_rmse = np.sqrt(mean_squared_error(y_test, plsr.predict(X_test)))
test_mae = mean_absolute_error(y_test, plsr.predict(X_test))
test_r2 = r2_score(y_test, plsr.predict(X_test))
print('TEST RESULTS: | RMSE {:.4f} | MAE {:.4f} | R2 {:.4f}'.format(test_rmse, test_mae, test_r2))

lucas_rmse = np.sqrt(mean_squared_error(y_lucas, plsr.predict(X_lucas)))
lucas_mae = mean_absolute_error(y_lucas, plsr.predict(X_lucas))
lucas_r2 = r2_score(y_lucas, plsr.predict(X_lucas))
print('LUCAS ZHOU2020 RESULTS: | RMSE {:.4f} | MAE {:.4f} | R2 {:.4f}'.format(lucas_rmse, lucas_mae, lucas_r2))

joblib.dump(plsr, "models/plsrmodel.joblib.pkl", compress=3)